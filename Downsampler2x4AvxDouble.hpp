...
#if ! defined (hiir_Downsampler2x4AvxDouble_CODEHEADER_INCLUDED)
#define hiir_Downsampler2x4AvxDouble_CODEHEADER_INCLUDED



/*\\\ INCLUDE FILES \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/

#include "hiir/StageProc4AvxDouble.h"
...
template <int NC>
Downsampler2x4AvxDouble <NC>::Downsampler2x4AvxDouble ()
:	_filter ()
{
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::set_coefs (const double coef_arr [])
{
	assert (coef_arr != 0);

	for (int i = 0; i < NBR_COEFS; ++i)
	{
		_mm256_store_pd (_filter [i + 2]._coef, _mm256_set1_pd (coef_arr [i]));
	}
}
...
template <int NC>
__m256d	Downsampler2x4AvxDouble <NC>::process_sample (const double in_ptr [8])
{
	assert (in_ptr != 0);

	const __m256d   in_0 = _mm256_loadu_pd (in_ptr    );
	const __m256d   in_1 = _mm256_loadu_pd (in_ptr + 4);
...
template <int NC>
__m256d	Downsampler2x4AvxDouble <NC>::process_sample (__m256d in_0, __m256d in_1)
{
	__m256d         spl_0 = in_1;
	__m256d         spl_1 = in_0;
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr != 0);
	assert (out_ptr != 0);
	...
	do
	{
		const __m256d   val = process_sample (in_ptr + pos * 8);
		_mm256_storeu_pd (out_ptr + pos * 4, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::process_sample_split (__m256d &low, __m256d &high, const double in_ptr [8])
{
	assert (in_ptr != 0);
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::process_sample_split (__m256d &low, __m256d &high, __m256d in_0, __m256d in_1)
{
	__m256d         spl_0 = in_1;
	__m256d         spl_1 = in_0;
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr != 0);
	assert (out_l_ptr != 0);
	...
	do
	{
		...
		_mm256_storeu_pd (out_l_ptr + pos * 4, low);
		_mm256_storeu_pd (out_h_ptr + pos * 4, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x4AvxDouble <NC>::clear_buffers ()
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
#endif   // hiir_Downsampler2x4AvxDouble_CODEHEADER_INCLUDED
