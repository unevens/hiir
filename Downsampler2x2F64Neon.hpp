...
template <int NC>
Downsampler2x2F64Neon <NC>::Downsampler2x2F64Neon ()
:	_filter ()
{
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::set_coefs (const double coef_arr [])
{
...
template <int NC>
float64x2_t	Downsampler2x2F64Neon <NC>::process_sample (const double in_ptr [4])
{
...
template <int NC>
float64x2_t	Downsampler2x2F64Neon <NC>::process_sample (float64x2_t in_0, float64x2_t in_1)
{
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr  != nullptr);
	...
	do
	{
		const float64x2_t val = process_sample (in_ptr + pos * 4);
		vst1q_f64 (out_ptr + pos * 2, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::process_sample_split (float64x2_t &low, float64x2_t &high, const double in_ptr [4])
{
	assert (in_ptr != nullptr);
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::process_sample_split (float64x2_t &low, float64x2_t &high, float64x2_t in_0, float64x2_t in_1)
{
	float64x2_t    spl_0 = in_1;
	float64x2_t    spl_1 = in_0;
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr    != nullptr);
	...
	do
	{
		...
		vst1q_f64 (out_l_ptr + pos * 2, low );
		vst1q_f64 (out_h_ptr + pos * 2, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x2F64Neon <NC>::clear_buffers ()
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
