...
#if ! defined (hiir_Downsampler2x2SseDouble_CODEHEADER_INCLUDED)
#define hiir_Downsampler2x2SseDouble_CODEHEADER_INCLUDED
...
template <int NC>
Downsampler2x2SseDouble <NC>::Downsampler2x2SseDouble ()
:	_filter ()
{
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::set_coefs (const double coef_arr [])
{
	assert (coef_arr != 0);
...
template <int NC>
__m128d	Downsampler2x2SseDouble <NC>::process_sample (const double in_ptr [4])
{
	assert (in_ptr != 0);

	const __m128d   in_0 = _mm_loadu_pd (in_ptr    );
	const __m128d   in_1 = _mm_loadu_pd (in_ptr + 2);
...
template <int NC>
__m128d	Downsampler2x2SseDouble <NC>::process_sample (__m128d in_0, __m128d in_1)
{
	__m128d         spl_0 = in_1;
	__m128d         spl_1 = in_0;
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr != 0);
	assert (out_ptr != 0);
	assert (out_ptr <= in_ptr || out_ptr >= in_ptr + nbr_spl * 4);
	assert (nbr_spl > 0);

	long           pos = 0;
	do
	{
		const __m128d   val = process_sample (in_ptr + pos * 4);
		_mm_storeu_pd (out_ptr + pos * 2, val);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::process_sample_split (__m128d &low, __m128d &high, const double in_ptr [4])
{
	assert (in_ptr != 0);
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::process_sample_split (__m128d &low, __m128d &high, __m128d in_0, __m128d in_1)
{
	__m128d         spl_0 = in_1;
	__m128d         spl_1 = in_0;
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::process_block_split (double out_l_ptr [], double out_h_ptr [], const double in_ptr [], long nbr_spl)
{
	assert (in_ptr != 0);
	assert (out_l_ptr != 0);
	assert (out_l_ptr <= in_ptr || out_l_ptr >= in_ptr + nbr_spl * 4);
	assert (out_h_ptr != 0);
	assert (out_h_ptr <= in_ptr || out_h_ptr >= in_ptr + nbr_spl * 4);
	...
	do
	{
		__m128d         low;
		__m128d         high;
		process_sample_split (low, high, in_ptr + pos * 4);
		_mm_storeu_pd (out_l_ptr + pos * 2, low);
		_mm_storeu_pd (out_h_ptr + pos * 2, high);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Downsampler2x2SseDouble <NC>::clear_buffers ()
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
#endif   // hiir_Downsampler2x2SseDouble_CODEHEADER_INCLUDED
