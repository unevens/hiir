...
#if ! defined (hiir_Upsampler2x4AvxDouble_CODEHEADER_INCLUDED)
#define hiir_Upsampler2x4AvxDouble_CODEHEADER_INCLUDED
...
template <int NC>
Upsampler2x4AvxDouble <NC>::Upsampler2x4AvxDouble ()
:	_filter ()
{
...
template <int NC>
void	Upsampler2x4AvxDouble <NC>::set_coefs (const double coef_arr [NBR_COEFS])
{
	assert (coef_arr != 0);
...
template <int NC>
void	Upsampler2x4AvxDouble <NC>::process_sample (__m256d &out_0, __m256d &out_1, __m256d input)
{
	__m256d         even = input;
	__m256d         odd  = input;
...
void	Upsampler2x4AvxDouble <NC>::process_block (double out_ptr [], const double in_ptr [], long nbr_spl)
{
	...
	do
	{
		__m256d         dst_0;
		__m256d         dst_1;
		const __m256d   src = _mm256_loadu_pd (in_ptr + pos * 4);
		process_sample (dst_0, dst_1, src);
		_mm256_storeu_pd (out_ptr + pos * 8    , dst_0);
		_mm256_storeu_pd (out_ptr + pos * 8 + 4, dst_1);
		++ pos;
	}
	while (pos < nbr_spl);
}
...
template <int NC>
void	Upsampler2x4AvxDouble <NC>::clear_buffers ()
{
	for (int i = 0; i < NBR_COEFS + 2; ++i)
	{
...
#endif   // hiir_Upsampler2x4AvxDouble_CODEHEADER_INCLUDED
